...
with st.form("user_form"):
    ...
    travel_preferences = st.multiselect("Travel Preferences", ["Nature", "Beach", "Mountains","Local experience", "Nightlife", "Historic places",  "Shopping", "Sightseeing", "Food", "Sports", "Music", "Exhibitions", "Street Walk"])
    hotel_preferences = st.multiselect("Hotel Preferences", ["Luxury", "Mid-Range", "Budget", "Boutique", "Business", "Resort", "Hostel"])
    food_preferences = st.multiselect("Food Preferences", ["Italian", "Chinese", "Indian", "Thai", "Japanese", "Mexican", "French", "Mediterranean", "Middle Eastern", "Vietnamese", "Korean", "American", "Local/Regional Cuisine", "Street Food"])
        ...
        if not selected_activities:
            ...
            if not user or not trip:
                ...
                for i, plan in enumerate(trip_plan):
                    ...
                    st.markdown(f"**📝 Notes:**\n\n{plan.special_notes}")
                    st.markdown(f"**Hotel Suggestions:**\n\n{plan.hotel_suggestions}")
                    st.markdown(f"**Restaurants Suggestions:**\n\n{plan.restaurants_suggestions}")
                    st.markdown("---")